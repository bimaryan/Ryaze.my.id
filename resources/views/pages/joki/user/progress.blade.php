                            ...
                            @forelse($activeOrders as $order)
                                <tr class="hover:bg-slate-50 dark:bg-slate-800/50 dark:hover:bg-slate-700/40 transition-colors">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span
                                            class="bg-slate-100 dark:bg-slate-700/50 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 text-xs px-2.5 py-1 rounded-md font-medium">
                                            {{ $order->tech_stack ?? 'Menunggu Info' }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 w-48 align-middle">
                                        <div class="flex justify-between text-xs mb-1.5">
                                            <span class="font-medium text-slate-500 dark:text-slate-400">Berjalan</span>
                                            <span class="font-bold text-indigo-600 dark:text-indigo-400">{{ $order->progress }}%</span>
                                        </div>
                                    ...
                                    <td class="px-6 py-4 text-center whitespace-nowrap">
                                        ...
                                        @elseif($order->status == 'progress')
                                            <span
                                                class="px-2.5 py-1 rounded-full text-xs font-medium bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-300 border border-blue-200 dark:border-blue-500/40">In
                                                Progress</span>
                                        @elseif($order->status == 'review')
                                            <span
                                                class="px-2.5 py-1 rounded-full text-xs font-medium bg-purple-50 dark:bg-purple-500/10 text-purple-600 dark:text-purple-300 border border-purple-200 dark:border-purple-500/40">Butuh
                                                Review</span>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 text-center whitespace-nowrap">
                                        <a href="{{ route('user_joki.detail', $order->hashid) }}"
                                            class="w-8 h-8 mx-auto rounded-lg flex items-center justify-center text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-500/10 hover:bg-indigo-600 hover:text-white transition-all duration-200 shadow-sm tooltip" title="Detail Proyek">
                                ...
                                <tr>
                                    <td colspan="5" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center justify-center">
                                            <div
                                                class="w-16 h-16 mb-4 bg-slate-100 dark:bg-slate-700/50 text-slate-400 dark:text-slate-500 rounded-full flex items-center justify-center text-2xl">
                                                <i class="fa-solid fa-folder-open"></i>
                                            </div>
                                            <p class="text-slate-500 dark:text-slate-400 font-medium">Anda belum memiliki proyek yang sedang
                                                berjalan.</p>
                                            <a href="{{ route('user_joki.create') }}"
                                                class="mt-2 text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 text-sm font-semibold hover:underline">
