    ...
    <x-ui.page-layout>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <x-ui.card class="p-6">
                    <h3 class="font-bold text-slate-800 dark:text-slate-100 mb-4">Progres Keseluruhan</h3>
                    <div class="w-full bg-slate-100 dark:bg-slate-700/50 rounded-full h-4 mb-3 overflow-hidden">
                        <div class="bg-indigo-600 h-4 rounded-full transition-all duration-500"
                            style="width: {{ $order->progress }}%"></div>
                    </div>
                    <p class="text-sm text-slate-600 dark:text-slate-300">Saat ini pengerjaan mencapai <strong
                            class="text-indigo-600 dark:text-indigo-400 text-base">{{ $order->progress }}%</strong></p>
                </x-ui.card>

                <x-ui.card class="p-6">
                    <h3 class="font-bold text-slate-800 dark:text-slate-100 mb-4 border-b pb-2">Target Pengerjaan (Milestone)</h3>
                    @if ($order->milestones->count() > 0)
                        <div class="space-y-4 relative border-l-2 border-slate-100 dark:border-slate-700 ml-3 pl-5">
                            @foreach ($order->milestones as $milestone)
                                <div class="relative">
                                    <div
                                        class="absolute -left-[29px] top-1 w-4 h-4 rounded-full border-2 border-white {{ $milestone->status == 'done' ? 'bg-emerald-500' : ($milestone->status == 'working' ? 'bg-blue-500' : 'bg-slate-300 dark:bg-slate-600') }}">
                                    </div>
                                    <h4 class="font-bold text-slate-800 dark:text-slate-100 text-sm">{{ $milestone->title }}</h4>
                                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-1 mb-1">{{ $milestone->description }}</p>
                                    <div class="flex gap-3 text-[11px] font-semibold mt-2">
                                        <span
                                            class="{{ $milestone->status == 'done' ? 'text-emerald-600 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-500/10' : ($milestone->status == 'working' ? 'text-blue-600 dark:text-blue-300 bg-blue-50 dark:bg-blue-500/10' : 'text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-700/50') }} px-2 py-1 rounded">
                                            Status: {{ strtoupper($milestone->status) }}
                                        </span>
                                        @if ($milestone->due_date)
                                            <span class="text-rose-600 dark:text-rose-300 bg-rose-50 dark:bg-rose-500/10 px-2 py-1 rounded"><i
                    ...
                    @else
                        <p class="text-sm text-slate-500 dark:text-slate-400 italic text-center py-4">Belum ada milestone yang ditambahkan oleh
                            Admin.</p>
                    @endif
                </x-ui.card>
                ...
                @if ($order->status == 'review' || $order->status == 'progress')
                    <x-ui.card class="p-6">
                        <h3 class="font-bold text-slate-800 dark:text-slate-100 mb-4 border-b pb-2">Ajukan Revisi</h3>
                        <form action="{{ route('user_joki.revision.store', $order->hashid) }}" method="POST">
                            ...
                            <textarea name="revision_note" rows="3" required placeholder="Jelaskan bagian mana yang perlu diperbaiki..."
                                class="mb-3 w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition"></textarea>
                            <button type="submit"
                        ...
                        @if ($order->revisions->count() > 0)
                            <div class="mt-6 space-y-3">
                                <h4 class="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Riwayat Revisi</h4>
                                @foreach ($order->revisions as $rev)
                                    <div class="bg-slate-50 dark:bg-slate-800/60 p-4 rounded-lg border border-slate-100 dark:border-slate-700 text-sm">
                                        <div class="flex justify-between items-start mb-2">
                                            <span class="font-semibold text-slate-800 dark:text-slate-100">Catatan Anda:</span>
                                            <span
                                                class="text-[10px] px-2 py-1 rounded font-bold uppercase {{ $rev->status == 'resolved' ? 'bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-300' : ($rev->status == 'fixing' ? 'bg-blue-100 dark:bg-blue-500/20 text-blue-700 dark:text-blue-300' : 'bg-amber-100 dark:bg-amber-500/20 text-amber-700 dark:text-amber-300') }}">
                                                {{ $rev->status }}
                                            </span>
                                        </div>
                                        <p class="text-slate-600 dark:text-slate-300 mb-3">{{ $rev->revision_note }}</p>
                                        @if ($rev->admin_reply)
                                            <div class="bg-indigo-50 dark:bg-indigo-500/10 p-3 rounded border border-indigo-100 dark:border-indigo-500/30">
                                                <span class="text-xs font-bold text-indigo-700 dark:text-indigo-300 block mb-1">Balasan
                                                    Admin:</span>
                                                <p class="text-indigo-900 dark:text-indigo-200 text-xs">{{ $rev->admin_reply }}</p>
                                            </div>
                                        @endif
                                    </div>
                                @endforeach
                ...
                @if ($order->status == 'completed')
                    <x-ui.card class="p-6">
                        <h3 class="font-bold text-slate-800 dark:text-slate-100 mb-4 border-b pb-2">Ulasan Anda</h3>
                        @if ($order->rating || $order->review)
                            <div class="bg-indigo-50 dark:bg-indigo-500/10 p-4 rounded-xl border border-indigo-100 dark:border-indigo-500/30">
                                <div class="flex items-center gap-1 text-amber-500 dark:text-amber-400 mb-2 text-lg">
                                    @for($i = 1; $i <= 5; $i++)
                                        <i class="fa-solid fa-star {{ $i <= $order->rating ? '' : 'text-slate-300 dark:text-slate-400' }}"></i>
                                    @endfor
                                </div>
                                <p class="text-sm text-slate-700 dark:text-slate-200 italic">"{{ $order->review }}"</p>
                            </div>
                        @else
                            <form action="{{ route('user_joki.review.store', $order->hashid) }}" method="POST">
                                @csrf
                                <div class="mb-4">
                                    <label class="block text-xs font-bold text-slate-700 dark:text-slate-200 mb-2">Rating (1-5)</label>
                                    <div class="flex items-center gap-4">
                                        @for($i = 5; $i >= 1; $i--)
                                        <label class="cursor-pointer text-center group">
                                            <input type="radio" name="rating" value="{{ $i }}" class="peer sr-only" required>
                                            <div class="w-10 h-10 rounded-full flex items-center justify-center border-2 border-slate-200 dark:border-slate-700 peer-checked:border-amber-500 peer-checked:bg-amber-50 group-hover:border-amber-300 transition-all">
                                                <span class="font-bold text-slate-500 dark:text-slate-400 peer-checked:text-amber-500">{{ $i }}</span>
                                            </div>
                                        </label>
                                        @endfor
                                    </div>
                                ...
                                <div class="mb-4">
                                    <label class="block text-xs font-bold text-slate-700 dark:text-slate-200 mb-2">Ulasan</label>
                                    <textarea name="review" rows="3" required placeholder="Bagaimana pengalaman Anda bekerja sama dengan kami?"
                                        class="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition"></textarea>
                                </div>
                                <button type="submit"
            ...
            <div class="space-y-6">
                    ...
                    <x-ui.card class="p-6 bg-slate-800 text-white border-none shadow-md">
                        <div class="flex items-center gap-3 mb-3">
                            <div class="w-10 h-10 bg-slate-700 rounded-full flex justify-center items-center">
                                <i class="fa-solid fa-eye text-emerald-400 dark:text-emerald-300"></i>
                            </div>
                            <div>
                                <h3 class="font-bold text-white text-sm">Live Preview</h3>
                                <p class="text-xs text-slate-400 dark:text-slate-500">Lihat hasil pengerjaan secara langsung.</p>
                            </div>
                        </div>
                            ...
                            @else
                                <form action="{{ route('user_joki.deploy_hosting', $order->hashid) }}" method="POST">
                                    ...
                                    <button type="submit" onclick="return confirm('Apakah Anda yakin ingin men-deploy project ini ke Ryaze Hosting? (Memerlukan penyimpanan aktif)')"
                                        class="w-full bg-white dark:bg-slate-800/60 text-indigo-700 dark:text-indigo-300 hover:bg-slate-50 dark:hover:bg-slate-700/40 font-bold px-4 py-2.5 rounded-lg text-sm shadow-md transition-colors flex justify-center items-center gap-2">
                ...
                <x-ui.card class="p-6">
                    <h3 class="font-bold text-slate-800 dark:text-slate-100 mb-4 border-b border-slate-100 dark:border-slate-700 pb-3">Tagihan Pembayaran</h3>

                    @if ($order->payments->count() > 0)
                        <div class="space-y-4">
                            @foreach ($order->payments as $payment)
                                <div
                                    class="border border-slate-200 dark:border-slate-700 rounded-lg p-4 {{ $payment->status == 'paid' ? 'bg-emerald-50/30 dark:bg-emerald-500/10' : 'bg-white dark:bg-slate-800/60' }}">
                                    <div class="flex justify-between items-center mb-2">
                                        <span class="font-bold text-slate-800 dark:text-slate-100 text-sm">{{ $payment->payment_name }}</span>
                                        <span
                                            class="text-xs px-2 py-1 rounded font-bold uppercase {{ $payment->status == 'paid' ? 'bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-300' : ($payment->status == 'unpaid' ? 'bg-rose-100 dark:bg-rose-500/20 text-rose-700 dark:text-rose-300' : 'bg-amber-100 dark:bg-amber-500/20 text-amber-700 dark:text-amber-300') }}">
                                            {{ $payment->status }}
                                        </span>
                                    </div>
                                    <div class="text-xl font-black text-slate-900 dark:text-slate-50 mb-3">Rp
                                    ...
                                    @else
                                        <!-- TAMPILAN JIKA LUNAS -->
                                        <p
                                            class="mt-2 text-xs text-emerald-600 dark:text-emerald-300 font-bold text-center bg-emerald-100 dark:bg-emerald-500/20 py-2 rounded-lg border border-emerald-200 dark:border-emerald-500/40">
                    ...
                    @else
                        <div class="text-center py-6">
                            <i class="fa-solid fa-file-invoice-dollar text-3xl text-slate-300 dark:text-slate-400 mb-2"></i>
                            <p class="text-sm text-slate-500 dark:text-slate-400">Belum ada tagihan dari Admin.</p>
                        </div>
                    @endif
                </x-ui.card>

                <x-ui.card class="p-6">
                    <h3 class="font-bold text-slate-800 dark:text-slate-100 mb-4 border-b border-slate-100 dark:border-slate-700 pb-3">Informasi Dasar</h3>
                    <div class="space-y-3 text-sm">
                        <div class="flex justify-between">
                            <span class="text-slate-500 dark:text-slate-400">Status</span>
                            <span class="font-bold text-indigo-600 dark:text-indigo-400 uppercase">{{ $order->status }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-slate-500 dark:text-slate-400">Tech Stack</span>
                            <span class="font-semibold text-slate-800 dark:text-slate-100">{{ $order->tech_stack ?? '-' }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-slate-500 dark:text-slate-400">Harga Total</span>
                            <span class="font-bold text-emerald-600 dark:text-emerald-300">Rp
    ...
    <div id="paymentModal" tabindex="-1" class="hidden fixed inset-0 z-[100] flex items-center justify-center w-full h-full bg-slate-900/50 backdrop-blur-sm p-4">
        <div class="relative bg-white dark:bg-slate-800/60 rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
            ...
            <div class="p-6 space-y-4">
                ...
                <!-- Option 1: Pakasir -->
                <a id="btnPakasir" href="#" target="_blank" onclick="closePaymentModal()" class="flex items-center justify-between p-4 border-2 border-slate-100 dark:border-slate-700 rounded-xl hover:border-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 transition-all group">
                    <div class="flex items-center gap-4">
