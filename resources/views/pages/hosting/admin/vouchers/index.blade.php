...
@section('content')
    <x-ui.page-layout>
        ...
        <x-ui.table>
                ...
                <tr class="hover:bg-slate-50 dark:bg-slate-800/50 dark:hover:bg-slate-700/40 transition-colors">
                    ...
                    <td class="px-6 py-4">
                        @if ($voucher->discount_percentage)
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-purple-100 dark:bg-purple-500/20 text-purple-800 dark:text-purple-200">
                                Diskon {{ $voucher->discount_percentage }}%
                            </span>
                        @elseif ($voucher->discount_amount)
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-blue-100 dark:bg-blue-500/20 text-blue-800 dark:text-blue-200">
                    ...
                    <td class="px-6 py-4 text-center">
                        @if ($voucher->isValid())
                            <span class="text-xs font-bold px-2.5 py-1 rounded-full bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-300">AKTIF</span>
                        @else
                            <span class="text-xs font-bold px-2.5 py-1 rounded-full bg-rose-100 dark:bg-rose-500/20 text-rose-700 dark:text-rose-300">TIDAK AKTIF</span>
                        @endif
                    </td>
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-2">
                            <a href="{{ route('admin_hosting.vouchers.edit', $voucher->hashid) }}" class="w-8 h-8 rounded-lg flex items-center justify-center text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-500/10 hover:bg-indigo-600 hover:text-white transition-all shadow-sm tooltip" title="Edit Voucher">
                            ...
                            <form action="{{ route('admin_hosting.vouchers.destroy', $voucher->hashid) }}" method="POST" class="inline-block" onsubmit="event.preventDefault(); let f = this; Swal.fire({title: 'Hapus Voucher?', text: 'Apakah Anda yakin ingin menghapus voucher ini?', icon: 'warning', showCancelButton: true, confirmButtonColor: '#ef4444', cancelButtonColor: '#6b7280', confirmButtonText: 'Ya, hapus!', cancelButtonText: 'Batal', customClass: {popup: 'rounded-2xl text-sm'}}).then(res => { if(res.isConfirmed) f.submit(); }); return false;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="w-8 h-8 rounded-lg flex items-center justify-center text-rose-600 dark:text-rose-300 bg-rose-50 dark:bg-rose-500/10 hover:bg-rose-600 hover:text-white transition-all shadow-sm tooltip" title="Hapus Voucher">
            ...
            @empty
                <tr>
                    <td colspan="6" class="px-6 py-10 text-center text-slate-400 dark:text-slate-500">Belum ada voucher yang dibuat.</td>
                </tr>
            @endforelse
        </x-ui.table>
