...
@section('content')
    <x-ui.page-layout>
        {{-- Header --}}
        <x-ui.page-header title="Kelola Tagihan Hosting" icon="file-invoice-dollar" iconColor="indigo" />
        ...
        <x-ui.table>
            ...
            @forelse ($payments as $payment)
                <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/40 transition-colors">
                    ...
                    <td class="px-6 py-4">
                        <div class="font-semibold text-slate-800 dark:text-slate-100">{{ $payment->project->project_name ?? 'Langganan Akun' }}</div>
                        <div class="text-xs text-slate-500 dark:text-slate-400">{{ $payment->user->name ?? ($payment->project->client->name ?? '-') }}</div>
                    </td>
                    <td class="px-6 py-4 uppercase text-xs font-semibold text-slate-500 dark:text-slate-400">
                    ...
                    <td class="px-6 py-4 text-right">
                        <button type="button"
                            class="btn-open-verify w-8 h-8 mx-auto rounded-lg flex items-center justify-center text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-500/10 hover:bg-indigo-600 hover:text-white transition-all duration-200 shadow-sm tooltip"
            ...
            @empty
                <tr>
                    <td colspan="6" class="px-6 py-10 text-center text-slate-400 dark:text-slate-500">Belum ada riwayat tagihan.</td>
                </tr>
            @endforelse
        </x-ui.table>
        ...
        {{-- Modal Verify --}}
        <div id="verifyModal"
            class="fixed inset-0 z-50 hidden bg-slate-900/50 flex items-center justify-center p-4 transition-opacity opacity-0 duration-300">
            <div
                class="bg-white dark:bg-slate-800/60 rounded-2xl shadow-xl w-full max-w-md overflow-hidden scale-95 transition-transform duration-300 transform">
                <div class="p-6 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between bg-slate-50/50 dark:bg-slate-800/50">
                    <h3 class="text-lg font-bold text-slate-800 dark:text-slate-100">Update Status Tagihan</h3>
                    <button type="button"
                        class="btn-close-verify text-slate-400 dark:text-slate-500 hover:text-rose-500 transition-colors p-2 rounded-lg hover:bg-rose-50 dark:hover:bg-rose-500/10">
                ...
                <form id="verifyForm" method="POST">
                    ...
                    @method('PUT')
                    <div class="p-6">
                        <p class="text-sm text-slate-500 dark:text-slate-400 mb-4">Ubah status untuk tagihan <strong id="verify_invoice"
                                class="text-slate-800 dark:text-slate-100"></strong>:</p>
                    ...
                    <div class="p-6 border-t border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 flex justify-end gap-3">
                        <button type="button"
                            class="btn-close-verify px-5 py-2.5 text-sm font-medium text-slate-600 dark:text-slate-300 bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700/40 transition-colors">
                            Batal
                        </button>
                        <button type="submit"
        ...
        <script nonce="{{ app('csp_nonce') ?? '' }}">
            (function() {
                ...
                const inner = modal.querySelector('div.bg-white dark:bg-slate-800/60');
