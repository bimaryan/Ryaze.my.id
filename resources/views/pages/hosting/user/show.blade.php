...
@section('content')
    <x-ui.page-layout>
        ...
        <div id="modal-create-email" class="fixed inset-0 z-50 flex items-center justify-center opacity-100 hidden transition-opacity duration-300">
            <div class="absolute inset-0 bg-slate-900/40 backdrop-blur-sm modal-overlay" onclick="document.getElementById('modal-create-email').classList.add('hidden')"></div>
            <div class="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 relative transform scale-100 transition-transform duration-300 ease-out z-10 flex flex-col max-h-[90vh]">
                <div class="flex items-center justify-between p-5 border-b border-slate-100 shrink-0">
                    ...
                    <button type="button" onclick="document.getElementById('modal-create-email').classList.add('hidden')" class="text-slate-400 hover:text-slate-600 transition-colors p-1 rounded-lg hover:bg-slate-100">
                        <i class="fa-solid fa-xmark text-lg"></i>
                    </button>
                </div>
                <div class="p-5 overflow-y-auto custom-scrollbar flex-1">
                    <form id="create-email-form" action="{{ route('user_hosting.emails.store') }}" method="POST" class="space-y-4">
                        @csrf
                        <div>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="text" name="password" id="email-password-input" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition pr-10 font-mono" placeholder="Minimal 8 karakter" required minlength="8">
                                <button type="button" onclick="document.getElementById('email-password-input').value = Math.random().toString(36).slice(-8) + Math.random().toString(36).slice(-8).toUpperCase()" class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-indigo-600 transition-colors" title="Generate Password">
                ...
                <div class="p-5 border-t border-slate-100 bg-slate-50/50 rounded-b-2xl shrink-0 flex justify-end gap-2">
                    <button type="button" onclick="document.getElementById('modal-create-email').classList.add('hidden')" class="px-4 py-2 text-sm font-medium text-slate-600 hover:bg-slate-200 bg-slate-100 rounded-xl transition-colors">Batal</button>
                    <button type="submit" form="create-email-form" class="px-5 py-2 text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl transition-all shadow-sm hover:shadow active:scale-[0.98]">Buat Email</button>
                </div>
            </div>
        </div>
