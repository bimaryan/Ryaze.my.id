        ...
        <div class="bg-slate-950 rounded-xl shadow-lg border border-slate-800 overflow-hidden flex flex-col mt-4" style="height: 600px;">
            <div class="bg-slate-900 border-b border-slate-800 px-4 py-3 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <span class="text-slate-400 dark:text-slate-500 text-xs font-mono">Terminal - {{ $build->package_name }}</span>
                </div>
                <div id="statusBadge">
                    @if($build->status === 'success')
                        <span class="text-xs font-bold px-2.5 py-1 rounded-full bg-emerald-900/50 text-emerald-400 dark:text-emerald-300 border border-emerald-800"><i class="fa-solid fa-check mr-1"></i> Selesai</span>
                    @elseif($build->status === 'failed')
                        <span class="text-xs font-bold px-2.5 py-1 rounded-full bg-rose-900/50 text-rose-400 dark:text-rose-300 border border-rose-800"><i class="fa-solid fa-xmark mr-1"></i> Gagal</span>
                    @else
            ...
            <pre id="logContent" class="flex-1 overflow-y-auto p-6 text-sm font-mono text-green-400 dark:text-green-300 leading-relaxed whitespace-pre-wrap focus:outline-none"></pre>
            ...
            <div id="downloadSection" class="bg-slate-900 border-t border-slate-800 p-4 flex justify-between items-center {{ $build->status === 'success' ? '' : 'hidden' }}">
                <p class="text-emerald-400 dark:text-emerald-300 text-sm"><i class="fa-solid fa-party-horn mr-1"></i> Build berhasil! APK Anda siap diunduh.</p>
                        ...
                        .then(data => {
                            ...
                            if (data.status === 'success' || data.status === 'failed') {
                                ...
                                if (data.status === 'success') {
                                    statusBadge.innerHTML = '<span class="text-xs font-bold px-2.5 py-1 rounded-full bg-emerald-900/50 text-emerald-400 dark:text-emerald-300 border border-emerald-800"><i class="fa-solid fa-check mr-1"></i> Selesai</span>';
                                    downloadSection.classList.remove('hidden');
                                } else {
                                    statusBadge.innerHTML = '<span class="text-xs font-bold px-2.5 py-1 rounded-full bg-rose-900/50 text-rose-400 dark:text-rose-300 border border-rose-800"><i class="fa-solid fa-xmark mr-1"></i> Gagal</span>';
                                }
                            }
                        })
