...
@section('content')
    <x-ui.page-layout>
        ...
        @if($activePromo)
            <div class="mt-6">
                <a href="{{ $activePromo->target_url ?? '#' }}" class="block relative rounded-2xl overflow-hidden shadow-sm group hover:shadow-md transition-all duration-300">
                    ...
                    @else
                        <div class="bg-gradient-to-r from-indigo-500 to-purple-600 px-6 py-8 sm:py-10 text-center">
                            <h3 class="text-2xl font-bold text-white tracking-tight">{{ $activePromo->title }}</h3>
