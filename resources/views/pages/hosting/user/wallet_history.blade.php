    ...
    <x-ui.page-layout>
        ...
        <div class="bg-gradient-to-r from-slate-800 to-slate-900 rounded-2xl p-6 text-white shadow-lg relative overflow-hidden mb-6">
            ...
            <div class="relative z-10">
                <div class="text-slate-300 dark:text-slate-400 text-sm font-semibold mb-1">Saldo Aktif</div>
        ...
        <x-ui.table>
            ...
            @forelse ($transactions as $tx)
                <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/40 transition-colors">
                    ...
                    <td class="px-6 py-4 text-sm font-medium text-slate-700 dark:text-slate-200">
                        {{ $tx->description }}
                        @if($tx->reference_id)
                            <div class="text-xs text-slate-400 dark:text-slate-500 font-mono mt-1">{{ $tx->reference_id }}</div>
                        @endif
                    </td>
                    <td class="px-6 py-4">
                        @if($tx->type === 'credit')
                            <span class="text-xs font-bold px-2 py-1 rounded bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-300"><i class="fa-solid fa-arrow-down mr-1"></i> MASUK</span>
                        @else
                            <span class="text-xs font-bold px-2 py-1 rounded bg-rose-100 dark:bg-rose-500/20 text-rose-700 dark:text-rose-300"><i class="fa-solid fa-arrow-up mr-1"></i> KELUAR</span>
                        @endif
                    </td>
                    <td class="px-6 py-4 font-mono font-medium @if($tx->type === 'credit') text-emerald-600 dark:text-emerald-300 @else text-rose-600 dark:text-rose-300 @endif">
            ...
            @empty
                <tr>
                    <td colspan="6" class="px-6 py-10 text-center text-slate-400 dark:text-slate-500">Belum ada riwayat transaksi dompet.</td>
                </tr>
            @endforelse
            <x-slot:pagination>
                {{ $transactions->links() }}
