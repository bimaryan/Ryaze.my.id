...
@section('content')
    <x-ui.page-layout>
        ...
        @if ($hasActive)
            @php $planColor = $colorMap[$plans[$currentPlan]['color'] ?? 'indigo']['badge'] ?? 'bg-indigo-100 text-indigo-700 border-indigo-200'; @endphp
        ...
        <div class="mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                @foreach ($plans as $slug => $plan)
                    ...
                    @endphp
                    <div class="relative bg-white rounded-2xl border {{ $isPopular ? 'border-violet-400 shadow-lg shadow-violet-100' : 'border-slate-200 shadow-sm' }} {{ $isCurrent ? 'opacity-80' : '' }} overflow-hidden flex flex-col">
                        @if ($isPopular)
                            <div class="absolute top-0 inset-x-0 h-1 bg-violet-500"></div>
                        @endif
                        
                        @if ($isCurrent)
                            <div class="absolute top-3 right-3"><span class="text-[10px] font-bold bg-emerald-100 text-emerald-700 border border-emerald-200 px-2 py-0.5 rounded-full">SAAT INI</span></div>
                        @elseif ($isPopular)
                        ...
                        <div class="px-6 pb-6">
                            @if ($isCurrent)
                                <button type="button" disabled class="w-full bg-slate-100 text-slate-400 font-bold px-4 py-2.5 rounded-xl text-sm cursor-not-allowed">
                                    Paket Aktif
                                </button>
                            @else
