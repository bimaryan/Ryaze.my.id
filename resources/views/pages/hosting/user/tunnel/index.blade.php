            ...
            <x-slot:actions>
                <button type="button" onclick="document.getElementById('documentationModal').classList.remove('hidden')"
                   class="inline-flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-800/60 text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700/40 border border-slate-200 dark:border-slate-700 text-sm font-medium rounded-lg transition-all shadow-sm">
                    <i class="fa-solid fa-book-open text-indigo-500 dark:text-indigo-400"></i>
                    Dokumentasi
                </button>
            ...
            <x-ui.card>
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-slate-50 dark:bg-slate-800/60 border-y border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 text-xs uppercase tracking-wider font-semibold">
                        ...
                        <tbody class="divide-y divide-slate-200 dark:divide-slate-700 text-sm">
                            @forelse($tunnels as $tunnel)
                                <tr class="hover:bg-slate-50/50 transition-colors">
                                    ...
                                    <td class="px-6 py-4">
                                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-indigo-50 dark:bg-indigo-500/10 text-indigo-700 dark:text-indigo-300 border border-indigo-100 dark:border-indigo-500/30">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex gap-2 justify-end">
                                            <a href="{{ route('user_hosting.tunnels.client', $tunnel->id) }}" target="_blank" class="w-8 h-8 rounded-lg bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-300 flex items-center justify-center hover:bg-emerald-100 dark:hover:bg-emerald-500/20 hover:text-emerald-700 dark:hover:text-emerald-300 transition-colors tooltip" data-tip="Download Client (.php)">
                                                <i class="fa-brands fa-php"></i>
                                            </a>
                                            <button type="button" onclick="showInstruction('{{ $tunnel->subdomain }}')" class="w-8 h-8 rounded-lg bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 flex items-center justify-center hover:bg-indigo-100 dark:hover:bg-indigo-500/20 dark:hover:bg-indigo-500/20 hover:text-indigo-700 dark:hover:text-indigo-400 dark:hover:text-indigo-300 transition-colors tooltip" data-tip="Cara Penggunaan">
                                            ...
                                            onsubmit="event.preventDefault(); let f = this; Swal.fire({title: 'Hapus Tunnel?', text: 'Apakah Anda yakin ingin menghapus tunnel ini? Subdomain akan dilepas permanen.', icon: 'warning', showCancelButton: true, confirmButtonColor: '#ef4444', cancelButtonColor: '#6b7280', confirmButtonText: '<i class=\'fa-solid fa-trash-can mr-1\'></i> Ya, Hapus', cancelButtonText: 'Batal', customClass: {popup: 'rounded-2xl text-sm'}}).then(res => { if(res.isConfirmed) f.submit(); }); return false;">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="w-8 h-8 rounded-lg bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-300 flex items-center justify-center hover:bg-red-100 dark:hover:bg-red-500/20 hover:text-red-700 dark:hover:text-red-300 transition-colors tooltip" data-tip="Hapus Tunnel">
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="px-6 py-12 text-center text-slate-500 dark:text-slate-400">
                                        <div class="w-16 h-16 rounded-full bg-slate-100 dark:bg-slate-700/50 flex items-center justify-center mx-auto mb-3">
                                            <i class="fa-solid fa-network-wired text-2xl text-slate-400 dark:text-slate-500"></i>
                                        </div>
                                        <p class="font-medium text-slate-600 dark:text-slate-300">Belum ada tunnel aktif</p>
            ...
            <div class="bg-white dark:bg-slate-800/60 rounded-2xl shadow-xl w-full max-w-lg overflow-hidden transition-transform duration-300 transform">
                ...
                <form action="{{ route('user_hosting.tunnels.store') }}" method="POST">
                    ...
                    <div class="p-6 space-y-4">
                        <div>
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">Nama Tunnel</label>
                            <input type="text" name="name" required placeholder="Misal: Proyek React Next.js"
                                   class="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">Subdomain</label>
                            <div class="flex items-center">
                                <input type="text" name="subdomain" required placeholder="my-app" pattern="[a-z0-9-]+" title="Hanya huruf kecil, angka, dan strip"
                                       class="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-l-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition">
                                <div class="px-4 py-2.5 bg-slate-100 dark:bg-slate-700/50 border border-l-0 border-slate-200 dark:border-slate-700 rounded-r-xl text-sm text-slate-500 dark:text-slate-400 font-medium">
                                    .ryaze.my.id
                                </div>
                            </div>
                            <p class="text-[11px] text-slate-500 dark:text-slate-400 mt-1.5"><i class="fa-solid fa-info-circle mr-1"></i> Hanya huruf kecil, angka, dan tanda hubung (-).</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">Target Port Localhost</label>
                            <div class="relative">
            ...
            <div class="bg-white dark:bg-slate-800/60 rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden transition-transform duration-300 transform">
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div class="flex items-start gap-4">
                        <div class="w-8 h-8 rounded-full bg-indigo-100 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 font-bold flex items-center justify-center shrink-0">1</div>
                        <div>
                            <h4 class="font-semibold text-slate-800 dark:text-slate-100 mb-1">Download Client</h4>
                            <p class="text-sm text-slate-600 dark:text-slate-300">Klik ikon <i class="fa-brands fa-php text-emerald-600 dark:text-emerald-300"></i> pada tabel untuk mengunduh script PHP.</p>
                        </div>
                    </div>

                    <div class="flex items-start gap-4">
                        <div class="w-8 h-8 rounded-full bg-indigo-100 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 font-bold flex items-center justify-center shrink-0">2</div>
                        <div class="w-full">
                            <h4 class="font-semibold text-slate-800 dark:text-slate-100 mb-1">Jalankan Tunnel</h4>
                            <p class="text-sm text-slate-600 dark:text-slate-300 mb-3">Jalankan script PHP yang telah diunduh dengan perintah ini di terminal:</p>
                    ...
                    <div class="flex items-start gap-4">
                        <div class="w-8 h-8 rounded-full bg-emerald-100 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-300 font-bold flex items-center justify-center shrink-0"><i class="fa-solid fa-check"></i></div>
                        <div>
                            <h4 class="font-semibold text-slate-800 dark:text-slate-100 mb-1">Selesai! Akses URL Public Anda</h4>
                            <p class="text-sm text-slate-600 dark:text-slate-300">Selama terminal Anda terbuka dan script berjalan, siapapun bisa mengakses localhost Anda melalui link: <br>
                            <a href="#" id="insLink" target="_blank" class="text-indigo-600 dark:text-indigo-400 hover:underline font-medium mt-1 inline-block"></a></p>
                        </div>
                    </div>
        ...
        <div id="documentationModal" class="hidden fixed inset-0 z-50 bg-slate-900/50 flex items-center justify-center p-4 transition-opacity duration-300">
            <div class="bg-white dark:bg-slate-800/60 rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden transition-transform duration-300 transform">
                ...
                <div class="p-6 overflow-y-auto space-y-8 flex-1">
                    ...
                    <!-- Section 1 -->
                    <div>
                        <h4 class="text-base font-bold text-slate-800 dark:text-slate-100 border-b border-slate-100 dark:border-slate-700 pb-2 mb-4">1. Apa itu Ryaze Local Tunnel?</h4>
                    ...
                    <!-- Section 2 -->
                    <div>
                        <h4 class="text-base font-bold text-slate-800 dark:text-slate-100 border-b border-slate-100 dark:border-slate-700 pb-2 mb-4">2. Alur Kerja (Bagaimana Ini Bekerja?)</h4>
                        <div class="relative pl-6 space-y-6 before:content-[''] before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-[2px] before:bg-indigo-100">
                            ...
                            <div class="relative">
                                ...
                                <div class="font-semibold text-slate-800 dark:text-slate-100 mb-1">Pengunjung Mengakses Link Anda</div>
                                <p class="text-sm text-slate-600 dark:text-slate-300">Seseorang membuka link publik Anda (contoh: <code class="text-xs bg-slate-100 dark:bg-slate-700/50 px-1 py-0.5 rounded text-slate-700 dark:text-slate-200">https://subdomain.ryaze.my.id</code>) di browser mereka.</p>
                            </div>
                            ...
                            <div class="relative">
                                ...
                                <div class="font-semibold text-slate-800 dark:text-slate-100 mb-1">Server Ryaze Meneruskan Request</div>
                                <p class="text-sm text-slate-600 dark:text-slate-300">Sistem Ryaze secara instan dan aman akan meneruskan permintaan (request) tersebut langsung ke terminal/CMD komputer Anda yang sedang menjalankan script client.</p>
                            </div>
                            ...
                            <div class="relative">
                                ...
                                <div class="font-semibold text-slate-800 dark:text-slate-100 mb-1">Script Mengakses Localhost</div>
                                <p class="text-sm text-slate-600 dark:text-slate-300">Script di komputer Anda secara otomatis akan memproses permintaan tersebut ke aplikasi localhost Anda (misal ke port 8000).</p>
                            </div>
                            ...
                            <div class="relative">
                                ...
                                <div class="font-semibold text-slate-800 dark:text-slate-100 mb-1">Tampil di Browser Pengunjung</div>
                                <p class="text-sm text-slate-600 dark:text-slate-300">Setelah aplikasi localhost Anda merespon, script mengirimkan hasilnya kembali ke jaringan Ryaze dan langsung ditampilkan di layar pengunjung dengan sangat cepat.</p>
                            </div>
                    ...
                    <div class="bg-blue-50/50 rounded-xl p-5 border border-blue-100 dark:border-blue-500/30">
                        ...
                        <div class="space-y-4 mt-4">
                            <div class="flex items-start">
                                <span class="inline-flex w-6 h-6 items-center justify-center rounded-full bg-blue-100 dark:bg-blue-500/20 text-blue-600 dark:text-blue-300 font-bold text-sm mr-2 shrink-0">1</span>
                                <div>
                                    <h4 class="font-medium text-slate-800 dark:text-slate-100">Download Client</h4>
                                    <p class="text-sm text-slate-600 dark:text-slate-300 mt-1">Klik tombol <strong>Download (.php)</strong> <i class="fa-brands fa-php text-emerald-600 dark:text-emerald-300 mx-1"></i> pada baris tunnel yang Anda buat.</p>
                                </div>
                            </div>
                            
                            <div class="flex items-start">
                                <span class="inline-flex w-6 h-6 items-center justify-center rounded-full bg-blue-100 dark:bg-blue-500/20 text-blue-600 dark:text-blue-300 font-bold text-sm mr-2 shrink-0">2</span>
                                <div>
                                    <h4 class="font-medium text-slate-800 dark:text-slate-100">Jalankan Client</h4>
                                    <p class="text-sm text-slate-600 dark:text-slate-300 mt-1">Jalankan perintah ini di terminal komputer Anda:</p>
                                </div>
                            </div>
                        </div>
                    </div>
