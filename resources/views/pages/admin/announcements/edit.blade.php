...
@section('content')
    <x-ui.page-layout>
        <x-ui.page-header 
            ...
            subtitle="Perbarui informasi atau pengumuman." 
        ...
        <x-ui.card class="w-full mt-6">
            <form action="{{ route('superadmin.announcements.update', $announcement->hashid) }}" method="POST" class="p-6">
                @csrf
                @method('PUT')
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="audience" id="audience" class="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition">
                                ...
                                <option value="hosting" {{ old('audience', $announcement->audience) === 'hosting' ? 'selected' : '' }}>Hosting Saja</option>
                                <option value="joki" {{ old('audience', $announcement->audience) === 'joki' ? 'selected' : '' }}>Joki Saja</option>
                            </select>
                    ...
                    <div class="flex flex-col sm:flex-row gap-4">
                        <label class="relative inline-flex items-center cursor-pointer">
                            ...
                            <input type="checkbox" name="is_active" value="1" class="sr-only peer" {{ $announcement->is_active ? 'checked' : '' }}>
                            ...
                            <span class="ml-3 text-sm font-medium text-slate-700 dark:text-slate-200">Aktif</span>
                        </label>

                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="hidden" name="is_pinned" value="0">
                            <input type="checkbox" name="is_pinned" value="1" class="sr-only peer" {{ $announcement->is_pinned ? 'checked' : '' }}>
                    ...
                    <div class="pt-4 border-t border-slate-100 dark:border-slate-700 flex justify-end">
                        <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2.5 px-6 rounded-lg shadow-md transition-colors">
                            <i class="fa-solid fa-save mr-2"></i> Perbarui Informasi
                        </button>
                    </div>
                </div>
            </form>
