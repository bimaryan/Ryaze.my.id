...
@section('content')
    <x-ui.page-layout>
        ...
        <x-ui.card class="w-full mt-6">
            <form action="{{ route('superadmin.testimonials.update', $testimonial->hashid) }}" method="POST" enctype="multipart/form-data" class="p-6">
                @csrf @method('PUT')
                <div class="space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $testimonial->name) }}" placeholder="Contoh: Andi Wijaya"
                        ...
                        <div>
                            ...
                            <input type="text" name="role" id="role" value="{{ old('role', $testimonial->role) }}" placeholder="Contoh: Product Manager"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            ...
                            <input type="text" name="company" id="company" value="{{ old('company', $testimonial->company) }}" placeholder="Contoh: PT Maju Jaya"
                    ...
                    <div>
                        ...
                        <textarea name="content" id="content" rows="4" placeholder="Tulis ulasan klien tentang kerja sama..."
                            class="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none @error('content') border-red-500 @enderror" required>{{ old('content', $testimonial->content) }}</textarea>
                    ...
                    <div>
                        ...
                        @if($testimonial->avatar_path)
                            <div class="mb-3 flex items-center gap-3">
                                <img src="{{ asset('storage/' . $testimonial->avatar_path) }}" alt="{{ $testimonial->name }}" class="w-12 h-12 rounded-full object-cover">
                                <span class="text-xs text-slate-500 dark:text-slate-400">Avatar saat ini</span>
                            </div>
                        @endif
