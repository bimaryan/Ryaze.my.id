        ...
        <div class="mt-6 space-y-6">
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                <form method="GET" action="{{ route('superadmin.finance') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                    ...
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">Layanan</label>
                        <select name="service"
                            class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition">
                    ...
                    <div class="md:col-span-4 flex flex-wrap items-center gap-2">
                        ...
                        <a href="{{ route('superadmin.finance') }}"
                            class="inline-flex items-center px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm font-bold rounded-xl transition">
                        ...
                        @foreach ($presets as $key => $preset)
                            ...
                            <a href="{{ route('superadmin.finance', array_filter(['from' => $preset['from'], 'to' => $preset['to']])) }}"
                                class="px-3 py-1.5 rounded-lg text-xs font-bold border transition {{ $active ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-slate-600 border-slate-200 hover:border-indigo-300 hover:text-indigo-600' }}">
                                {{ $preset['label'] }}
                            </a>
                        @endforeach
                    </div>
            ...
            <div class="bg-indigo-50 border border-indigo-100 rounded-xl px-5 py-3 text-sm text-indigo-800 flex items-start gap-3">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 relative overflow-hidden">
                    ...
                    <div class="relative z-10">
                        ...
                        <h3 class="text-2xl font-bold text-slate-800">Rp {{ number_format($totalRevenue, 0, ',', '.') }}</h3>
                        <p class="text-xs text-slate-400 mt-1">{{ number_format($totalCount) }} transaksi lunas</p>
                    </div>
                </div>
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 relative overflow-hidden">
                    <div class="absolute -right-4 -top-4 w-24 h-24 bg-indigo-50 rounded-full"></div>
                    <div class="relative z-10">
                        <p class="text-sm font-medium text-slate-500 mb-1">Pendapatan Joki Code</p>
                        <h3 class="text-2xl font-bold text-indigo-700">Rp {{ number_format($jokiRevenue, 0, ',', '.') }}</h3>
                        <p class="text-xs text-slate-400 mt-1">{{ number_format($jokiCount) }} transaksi lunas</p>
                    </div>
                </div>
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 relative overflow-hidden">
                    <div class="absolute -right-4 -top-4 w-24 h-24 bg-emerald-50 rounded-full"></div>
                    <div class="relative z-10">
                        <p class="text-sm font-medium text-slate-500 mb-1">Pendapatan Hosting</p>
                        <h3 class="text-2xl font-bold text-emerald-700">Rp {{ number_format($hostingRevenue, 0, ',', '.') }}</h3>
                        <p class="text-xs text-slate-400 mt-1">{{ number_format($hostingCount) }} transaksi lunas</p>
                    </div>
                </div>
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 relative overflow-hidden">
                    <div class="absolute -right-4 -top-4 w-24 h-24 bg-amber-50 rounded-full"></div>
                    <div class="relative z-10">
                        <p class="text-sm font-medium text-slate-500 mb-1">Transaksi Per Metode</p>
                        <div class="space-y-1.5 mt-1">
                            @forelse($methods->take(3) as $m => $data)
                                <div class="flex justify-between text-sm">
                                    <span class="font-medium text-slate-600">{{ $m }}</span>
                                    <span class="font-bold text-slate-800">Rp {{ number_format($data['total'], 0, ',', '.') }}</span>
                                </div>
                            @empty
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div class="lg:col-span-2 bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                    <h3 class="text-lg font-bold text-slate-800 mb-4">Perbandingan Pendapatan Joki vs Hosting (12 Bulan Terakhir)</h3>
                    <div id="chart-finance-monthly"></div>
                </div>
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                    <h3 class="text-lg font-bold text-slate-800 mb-4">Distribusi Metode Pembayaran</h3>
                    ...
                    <div class="mt-4 space-y-2">
                        @forelse($methods as $m => $data)
                            <div class="flex justify-between text-sm border-b border-slate-50 pb-1.5">
                                <span class="font-medium text-slate-600">{{ $m }}</span>
                                <span class="text-slate-500">{{ number_format($data['count']) }}x &middot; <span class="font-bold text-slate-800">Rp {{ number_format($data['total'], 0, ',', '.') }}</span></span>
                            </div>
                        @empty
                ...
                <x-ui.table>
                    ...
                    @forelse ($transactions as $row)
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-xs font-mono font-semibold text-slate-700">
                                {{ $row['invoice'] }}
                            </td>
                            ...
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-semibold {{ $row['source'] === 'joki' ? 'bg-indigo-50 text-indigo-700 border border-indigo-100' : 'bg-emerald-50 text-emerald-700 border border-emerald-100' }}">
                                    {{ $row['source'] === 'joki' ? 'Joki' : 'Hosting' }}
                                </span>
                                <span class="text-xs text-slate-500 ms-2">{{ $row['detail'] }}</span>
                            </td>
                            <td class="px-6 py-4">
                                <span class="text-xs font-medium bg-slate-100 text-slate-700 px-2.5 py-1 rounded-md border border-slate-200">
                                    {{ $row['method'] }}
                                </span>
                            </td>
