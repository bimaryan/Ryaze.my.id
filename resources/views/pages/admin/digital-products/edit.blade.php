...
@section('content')
    <x-ui.page-layout>
        <x-ui.page-header 
            ...
            subtitle="Ubah data produk digital." 
        ...
        <x-ui.card class="w-full mt-6">
            <form action="{{ route('superadmin.digital_products.update', $product->hashid) }}" method="POST" enctype="multipart/form-data" class="p-6">
                @csrf @method('PUT')
                <div class="space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <label for="slug" class="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-2">Slug</label>
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3"
                            class="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none">{{ old('description', $product->description) }}</textarea>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="number" name="price" id="price" value="{{ old('price', $product->price) }}" min="1000" step="1000"
                        ...
                        <div>
                            ...
                            @if($product->cover_path)
                                <div class="mb-3">
                                    <img src="{{ asset('storage/' . $product->cover_path) }}" alt="{{ $product->name }}" class="w-20 h-20 rounded-lg object-cover border border-slate-200 dark:border-slate-700">
                                </div>
                            @endif
                    ...
                    <div>
                        <label for="file" class="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-2">Ganti File Produk <span class="text-slate-400 font-normal">(kosongkan jika tidak diganti)</span></label>
                        <p class="text-xs text-slate-500 dark:text-slate-400 mb-2">
                            File saat ini: <span class="font-semibold">{{ $product->file_name }}</span> ({{ $product->download_count }} download)
                        </p>
