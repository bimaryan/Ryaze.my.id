...
@section('content')
    <x-ui.page-layout>
        ...
        <x-ui.card class="w-full mt-6">
            <form action="{{ route('admin.promo_events.update', $promo_event->hashid) }}" method="POST" enctype="multipart/form-data" class="p-6">
                @csrf
                @method('PUT')
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        @if($promo_event->banner_image)
                            <div class="mb-3">
                                ...
                                <img src="{{ $promo_event->banner_url }}" alt="Banner" class="w-48 object-cover rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm">
                            </div>
                        @endif
                        ...
                        <p class="text-xs text-slate-500 dark:text-slate-400 mt-2">Format: JPG, PNG, WEBP. Kosongkan jika tidak ingin mengubah banner.</p>
                        @error('banner_image') <p class="text-red-500 dark:text-red-400 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="is_active" value="1" class="sr-only peer" {{ old('is_active', $promo_event->is_active) ? 'checked' : '' }}>
                            ...
                            <span class="ml-3 text-sm font-medium text-slate-700 dark:text-slate-200">Aktifkan promo ini</span>
                        </label>
                    </div>
