            ...
            <div class="flex flex-col sm:flex-row justify-between items-center mb-4 px-1 gap-4">
                ...
                <form action="{{ route('superadmin.portfolios.index') }}" method="GET" class="flex items-center w-full sm:w-auto">
                    ...
                    @if(request()->has('search') && request()->search != '')
                        <a href="{{ route('superadmin.portfolios.index') }}" class="p-2 ms-2 text-sm font-medium text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-700/50 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700 transition">
                            Reset
                        </a>
                    @endif
                </form>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                @if($portfolio->image_path)
                                    <img src="{{ Storage::url($portfolio->image_path) }}" alt="{{ $portfolio->title }}" class="w-12 h-12 object-cover rounded-lg border border-slate-200 dark:border-slate-700">
                                @else
                                ...
                                <div class="flex flex-col min-w-0">
                                    <span class="font-medium text-slate-800 dark:text-slate-100 truncate max-w-[250px]">{{ $portfolio->title }}</span>
                                    @if($portfolio->link_preview)
                                        <a href="{{ $portfolio->link_preview }}" target="_blank" class="text-[10px] text-indigo-500 dark:text-indigo-400 hover:underline mt-0.5 font-bold"><i class="fa-solid fa-link mr-1"></i>Live Preview</a>
                                    @endif
                                </div>
                            </div>
                        </td>
                            ...
                            @if($portfolio->tags)
                                <div class="flex flex-wrap gap-1">
                                    @foreach($portfolio->tags as $tag)
                                        <span class="px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-slate-100 dark:bg-slate-700/50 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700">{{ $tag }}</span>
                                    @endforeach
                                </div>
                            @else
                                <span class="text-slate-400 dark:text-slate-500 italic text-xs">Tanpa tag</span>
                            @endif
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <form action="{{ route('superadmin.portfolios.status.toggle', $portfolio->hashid) }}" method="POST" class="inline">
                                    @csrf @method('PATCH')
                                    <button type="submit" title="{{ $portfolio->is_active ? 'Jadikan Draft' : 'Aktifkan' }}" class="p-1.5 rounded-lg transition {{ $portfolio->is_active ? 'text-emerald-500 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 hover:bg-emerald-100 dark:hover:bg-emerald-500/20' : 'text-slate-400 dark:text-slate-500 hover:text-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-500/10' }}">
                                        <i class="fa-solid {{ $portfolio->is_active ? 'fa-eye' : 'fa-eye-slash' }}"></i>
                                    </button>
                                </form>
                                <a href="{{ route('superadmin.portfolios.edit', $portfolio->hashid) }}" class="p-1.5 text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-500/10 hover:bg-indigo-100 dark:hover:bg-indigo-500/20 dark:hover:bg-indigo-500/20 rounded-lg transition">
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-12 text-center text-slate-500 dark:text-slate-400">
                            <i class="fa-solid fa-briefcase text-3xl mb-3 text-slate-300 dark:text-slate-400"></i>
        ...
        <script>
            function confirmDelete(button) {
                Swal.fire({
                    ...
                    text: 'Data portofolio ini akan dihapus permanen dan tidak dapat dikembalikan.',
                    icon: 'warning',
