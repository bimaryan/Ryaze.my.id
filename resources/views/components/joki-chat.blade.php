...
<div x-data="jokiChat()" x-init="initChat()" class="mt-8 flex flex-col bg-white dark:bg-slate-800 border border-slate-200 rounded-2xl overflow-hidden shadow-sm h-[500px]">
    <div class="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 px-6 py-4 flex justify-between items-center">
    ...
    <!-- Area Pesan -->
    <div id="chat-box" class="flex-1 p-6 overflow-y-auto space-y-4 bg-slate-50 dark:bg-slate-800/50/50">
    ...
    <!-- Area Input -->
    <div class="bg-white dark:bg-slate-800 border-t border-slate-200 p-4">
        <form @submit.prevent="sendMessage" class="flex gap-2">
            <input type="text" x-model="newMessage" placeholder="Ketik pesan Anda di sini..." required
                   class="flex-1 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition disabled:opacity-50" :disabled="sending">
