...
export default function Finance() {
    const { payments, totalRevenue } = usePage().props;
            ...
            <div className="space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div className="bg-white dark:bg-[#0d0d18] border border-[#e5e5e5] dark:border-[#1a1a2e] p-5">
                        <div className="flex items-center gap-3 mb-3">
                            <div className="w-9 h-9 bg-[#7c3aed] flex items-center justify-center">
                                <i className="fa-solid fa-chart-line text-white text-sm"></i>
                            </div>
                            <span className="text-[12px] font-semibold text-[#999] dark:text-white/40 uppercase tracking-wider">Total Pendapatan</span>
                        </div>
                        <p className="text-xl font-black text-[#333] dark:text-white">{formatCurrency(totalRevenue)}</p>
                    </div>
                </div>
                ...
                <div className="bg-white dark:bg-[#0d0d18] border border-[#e5e5e5] dark:border-[#1a1a2e]">
                    <div className="px-5 py-4 border-b border-[#e5e5e5] dark:border-[#1a1a2e]">
                        <h2 className="text-sm font-bold text-[#333] dark:text-white">Pembayaran</h2>
                    </div>
                        ...
                        <table className="w-full text-left">
                            <thead>
                                <tr className="border-b border-[#e5e5e5] dark:border-[#1a1a2e]">
                                    ...
                                    <th className="px-5 py-3 text-[11px] font-bold text-[#999] dark:text-white/40 uppercase tracking-wider">Paid At</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-[#e5e5e5] dark:divide-[#1a1a2e]">
                                {payments?.data?.length > 0 ? payments.data.map((payment) => (
                                    <tr key={payment.id} className="hover:bg-[#fafafa] dark:hover:bg-white/[0.02] transition-colors">
                                        <td className="px-5 py-3 text-[13px] font-mono text-[#666] dark:text-white/60">{payment.invoice_number || '-'}</td>
                                        <td className="px-5 py-3 text-[13px] text-[#333] dark:text-white">{payment.user?.name || '-'}</td>
                                        <td className="px-5 py-3 text-[13px] text-[#666] dark:text-white/60">{payment.order?.service?.name || '-'}</td>
                                        <td className="px-5 py-3 text-[13px] font-semibold text-[#333] dark:text-white">{formatCurrency(payment.amount)}</td>
                                        <td className="px-5 py-3 text-[13px] text-[#666] dark:text-white/60">{payment.method || '-'}</td>
                                        <td className="px-5 py-3">{statusBadge(payment.status)}</td>
                                        <td className="px-5 py-3 text-[13px] text-[#999] dark:text-white/40">
                                            {payment.paid_at ? new Date(payment.paid_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' }) : '-'}
                                        </td>
                                    </tr>
                                )) : (
                                    <tr>
