...
export default function Index() {
    ...
    return (
        <DashboardLayout title="Tiket - Admin">
                                ...
                                {tickets?.data?.length > 0 ? tickets.data.map((ticket) => (
                                    <tr key={ticket.id} className="hover:bg-[#fafafa] dark:hover:bg-white/[0.02] transition-colors">
                                        <td className="px-5 py-3">
                                            <Link href={`/admin/tickets/${ticket.hashid || ticket.id}`} className="text-[13px] font-semibold text-[#333] dark:text-white hover:text-[#7c3aed] transition-colors">
                                                {ticket.subject}
                                            </Link>
                                        </td>
                                        <td className="px-5 py-3 text-[13px] text-[#666] dark:text-white/60">{ticket.user?.name || '-'}</td>
