...
export default function Show() {
    ...
    const handleReply = (e) => {
        ...
        post(`/user/tickets/${ticket.hashid || ticket.id}/reply`, {
    ...
    const allMessages = [
        { id: 'ticket', sender: ticket.user, message: ticket.message, created_at: ticket.created_at },
        ...(ticket.replies || []),
    ];
        ...
        <DashboardLayout title={`Tiket - ${ticket.subject}`}>
            <div className="max-w-3xl mx-auto space-y-6">
                <div className="bg-white dark:bg-[#0d0d18] border border-[#e5e5e5] dark:border-[#1a1a2e]">
                    <div className="px-5 py-4 border-b border-[#e5e5e5] dark:border-[#1a1a2e] flex items-center justify-between">
                        <div>
                            <div className="flex items-center gap-2">
                                <Link href="/user/tickets" className="text-[13px] text-[#999] dark:text-white/40 hover:text-[#7c3aed] transition-colors">
                    ...
                    {ticket.status === 'closed' && (
                        <div className="border-t border-[#e5e5e5] dark:border-[#1a1a2e] p-5 text-center">
                            <p className="text-[13px] text-[#999] dark:text-white/40">
                                <i className="fa-solid fa-lock mr-1"></i>Tiket ini telah ditutup
                            </p>
                        </div>
                    )}
                </div>
