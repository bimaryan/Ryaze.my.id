            ...
            <div className="space-y-6">
                <div className="bg-white dark:bg-[#0d0d18] border border-[#e5e5e5] dark:border-[#1a1a2e]">
                    <div className="px-5 py-4 border-b border-[#e5e5e5] dark:border-[#1a1a2e]">
                        <h2 className="text-sm font-bold text-[#333] dark:text-white">Billing History</h2>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-[#e5e5e5] dark:divide-[#1a1a2e]">
                                {billings?.data?.length > 0 ? billings.data.map((b) => (
                                    <tr key={b.id} className="hover:bg-[#fafafa] dark:hover:bg-white/[0.02] transition-colors">
                                        <td className="px-5 py-3 text-[13px] font-mono text-[#666] dark:text-white/60">{b.invoice_number || b.id}</td>
                                        <td className="px-5 py-3 text-[13px] font-semibold text-[#333] dark:text-white">{formatCurrency(b.amount)}</td>
                                        <td className="px-5 py-3">{statusBadge(b.status)}</td>
                                        <td className="px-5 py-3 text-[13px] text-[#666] dark:text-white/60">{b.method || '-'}</td>
                                        ...
                                        <td className="px-5 py-3 text-right">
                                            {b.status === 'pending' && (
                                                <Link href={`/user/hosting/billing/${b.id}/pay`} className="text-[12px] text-[#7c3aed] hover:text-[#6d28d9] font-medium transition-colors">
                                                    Pay Now
                                                </Link>
                                            )}
                                            {b.invoice_url && (
                                                <Link href={b.invoice_url} target="_blank" className="text-[12px] text-[#666] dark:text-white/60 hover:text-[#7c3aed] font-medium transition-colors ml-3">
                                    ...
                                    <tr>
                                        <td colSpan="6" className="px-5 py-12 text-center">
                                            ...
                                            <p className="text-[13px] text-[#999] dark:text-white/40">No billing records</p>
                                        </td>
                                    </tr>
                                )}
                            </tbody>
                    ...
                    {billings?.last_page > 1 && (
                        <div className="px-5 py-3 border-t border-[#e5e5e5] dark:border-[#1a1a2e] flex items-center justify-between">
                            <p className="text-[12px] text-[#999] dark:text-white/40">
                                Showing {billings.from}-{billings.to} of {billings.total}
                            </p>
                            <div className="flex items-center gap-1">
                                {billings.prev_page_url && (
                                    <Link href={billings.prev_page_url} preserveState className="px-3 py-1.5 text-[12px] font-medium border border-[#e5e5e5] dark:border-[#1a1a2e] text-[#666] dark:text-white/60 hover:border-[#7c3aed] hover:text-[#7c3aed] transition-colors">Prev</Link>
                                )}
                                {[...Array(billings.last_page)].map((_, i) => (
                                    <Link key={i + 1} href={`${billings.path}?page=${i + 1}`} preserveState className={`w-8 h-8 flex items-center justify-center text-[12px] font-medium border transition-colors ${billings.current_page === i + 1 ? 'bg-[#7c3aed] text-white border-[#7c3aed]' : 'border-[#e5e5e5] dark:border-[#1a1a2e] text-[#666] dark:text-white/60 hover:border-[#7c3aed] hover:text-[#7c3aed]'}`}>{i + 1}</Link>
                                ))}
                                {billings.next_page_url && (
                                    <Link href={billings.next_page_url} preserveState className="px-3 py-1.5 text-[12px] font-medium border border-[#e5e5e5] dark:border-[#1a1a2e] text-[#666] dark:text-white/60 hover:border-[#7c3aed] hover:text-[#7c3aed] transition-colors">Next</Link>
                                )}
                            </div>
                        </div>
                    )}
