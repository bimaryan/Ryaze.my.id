...
const planColors = {
    violet: {
        card: 'bg-[#1a1025] text-white border-[#2d1f42]',
        badge: 'bg-[#7c3aed]',
        btn: 'bg-white text-[#1a1025]',
        check: 'text-[#a78bfa]',
        popular: true,
    },
    default: {
    ...
    return (
        <div className="border-b border-[#e5e5e5]">
            <button onClick={() => setOpen(!open)} className="w-full flex items-center justify-between py-5 text-left group">
                <span className="text-[15px] font-medium text-[#1a1025] group-hover:text-[#7c3aed] transition-colors">{q}</span>
...
export default function Home({ plans, planPricing, articles, starterPricing }) {
    ...
    return (
        <PublicLayout title="Jasa Pembuatan Website & Shared Hosting Indonesia" description="Platform hosting & development modern.">
            ...
            <section className="relative bg-[#fafafa] overflow-hidden">
                ...
                <div className="relative max-w-6xl mx-auto px-6 pt-32 pb-28 lg:pt-44 lg:pb-36">
                    <div className="max-w-3xl" data-reveal>
                        ...
                        <h1 className="text-[clamp(2.5rem,6vw,4.5rem)] font-black text-[#1a1025] leading-[1.05] tracking-[-0.03em] mb-6">
                            Bangun produk digital,<br />
                            <span className="text-[#7c3aed]">deploy dalam hitungan menit.</span>
                        </h1>
                        <p className="text-[#666] text-lg max-w-xl mb-10 leading-relaxed">
                            Jasa pembuatan website & aplikasi. Shared hosting Indonesia dengan auto-deploy dari Git, SSL gratis, database MySQL.
                        </p>
                        <div className="flex flex-wrap gap-3">
                            <a href="#pricing" className="px-7 py-3 bg-[#1a1025] text-white text-sm font-semibold hover:bg-[#2d1f42] transition-colors">
                                Lihat Paket
                            </a>
                            <a href="#services" className="px-7 py-3 border border-[#ddd] text-[#1a1025] text-sm font-semibold hover:border-[#7c3aed] hover:text-[#7c3aed] transition-colors">
                                Pelajari Lebih Lanjut
                            </a>
                        </div>
                    </div>

                    <div className="mt-20 grid grid-cols-3 gap-px bg-[#e5e5e5] border border-[#e5e5e5]" data-reveal>
                        {[
                            { n: '99.9%', l: 'Uptime Server' },
                            { n: '100+', l: 'Project Selesai' },
                            { n: '< 5 mnt', l: 'Auto-Deploy' },
                        ].map((s, i) => (
                            <div key={i} className="bg-[#fafafa] px-6 py-8 text-center">
                                <p className="text-3xl font-black text-[#1a1025] tracking-tight">{s.n}</p>
                                <p className="text-[11px] font-bold text-[#999] mt-2 uppercase tracking-widest">{s.l}</p>
                            </div>
                        ))}
                    </div>

                    <div className="mt-16 flex items-center gap-8" data-reveal>
                        <span className="text-[10px] font-bold text-[#bbb] uppercase tracking-[0.2em]">Tech Stack</span>
                        <div className="flex gap-6 text-[#ccc]">
            ...
            {/* ═══ ABOUT ═══ */}
            <section id="about" className="bg-white border-y border-[#e5e5e5]">
                <div className="max-w-6xl mx-auto px-6 py-24 lg:py-32">
                    <div className="grid lg:grid-cols-12 gap-16 items-start">
                        <div className="lg:col-span-5" data-reveal>
                            ...
                            <h2 className="text-4xl font-black text-[#1a1025] tracking-tight leading-[1.1] mb-6">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {['AUTO DEPLOY', 'SSL GRATIS', 'MYSQL', 'TERMINAL'].map(t => (
                                    <span key={t} className="px-3 py-1 bg-[#f5f0ff] text-[#7c3aed] text-[11px] font-bold tracking-wider border border-[#e9e0ff]">{t}</span>
                                ))}
                            </div>
                        </div>
                            ...
                            <div className="border border-[#e5e5e5] bg-[#fafafa]">
                                <div className="border-b border-[#e5e5e5] px-8 py-6 flex items-center justify-between">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <h3 className="font-bold text-[#1a1025] text-lg">Ryaze Hosting</h3>
                                            <p className="text-[11px] text-[#999] font-medium uppercase tracking-wider">Shared Server Infrastructure</p>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-2 text-xs font-semibold text-emerald-600 bg-emerald-50 px-3 py-1.5 border border-emerald-100">
                                ...
                                <div className="grid grid-cols-3 divide-x divide-[#e5e5e5]">
                                    {[
                                        ...
                                        <div key={i} className="px-6 py-6 text-center">
                                            <p className="text-2xl font-black text-[#1a1025]">{s.v}</p>
                                            <p className="text-[10px] text-[#999] font-bold uppercase tracking-widest mt-1">{s.l}</p>
                                        </div>
                                    ))}
                                </div>
                                <div className="border-t border-[#e5e5e5] px-8 py-4 flex items-center gap-6 text-xs text-[#999]">
            ...
            <section id="services" className="bg-[#fafafa]">
                <div className="max-w-6xl mx-auto px-6 py-24 lg:py-32">
                    <div className="mb-16" data-reveal>
                        ...
                        <h2 className="text-4xl font-black text-[#1a1025] tracking-tight">Infrastruktur & Layanan.</h2>
                    </div>

                    <div className="grid md:grid-cols-2 gap-px bg-[#e5e5e5] border border-[#e5e5e5]" data-reveal>
                        {/* Dev */}
                        <div className="bg-white p-8 lg:p-10 flex flex-col">
                            <div className="w-10 h-10 bg-[#1a1025] flex items-center justify-center mb-8">
                                <i className="fa-solid fa-code text-white text-sm"></i>
                            </div>
                            <h3 className="text-2xl font-black text-[#1a1025] mb-3">Jasa Pembuatan Sistem</h3>
                            <p className="text-[#666] text-[15px] leading-relaxed mb-8 flex-1">
                                Sistem informasi, aplikasi SaaS, website korporat, prototipe fungsional. Arsitektur modern, bersih, terdokumentasi.
                            </p>
                            <ul className="space-y-3 mb-8 text-sm text-[#444]">
                                {['Backend & API Design', 'Frontend Modern (React / Vue)', 'Keamanan & Skalabilitas'].map(f => (
                                    <li key={f} className="flex items-center gap-3">
                                        <span className="w-4 h-4 bg-[#f5f0ff] flex items-center justify-center flex-shrink-0"><i className="fa-solid fa-check text-[8px] text-[#7c3aed]"></i></span>
                                        {f}
                                    </li>
                                ))}
                            </ul>
                            <a href="/consultation" className="inline-flex items-center justify-center px-6 py-2.5 border border-[#1a1025] text-[#1a1025] text-sm font-semibold hover:bg-[#1a1025] hover:text-white transition-colors w-full sm:w-auto">
                                Mulai Konsultasi
                            </a>
                        </div>

                        {/* Hosting */}
                        <div className="bg-white p-8 lg:p-10 flex flex-col relative overflow-hidden">
                            <div className="absolute top-0 right-0 w-32 h-32 bg-[#f5f0ff] rounded-bl-[80px]"></div>
                            ...
                            <div className="relative z-10">
                                <div className="flex items-end justify-between mb-3">
                                    <h3 className="text-2xl font-black text-[#1a1025]">Shared App Hosting</h3>
                                    <div className="text-right">
                                        {starterPricing.promo > 0 && <span className="text-xs text-[#999] price-strike block">Rp {rupiah(starterPricing.normal)}</span>}
                                        <div className="flex items-baseline gap-1">
                                            <span className="text-2xl font-black text-[#1a1025]">Rp {rupiah(starterPricing.active)}</span>
                                            <span className="text-xs text-[#999]">/bln</span>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <ul className="space-y-3 mb-8 text-sm text-[#444] relative z-10">
            ...
            <section id="pricing" className="bg-white border-y border-[#e5e5e5]">
                <div className="max-w-6xl mx-auto px-6 py-24 lg:py-32">
                    <div className="mb-16" data-reveal>
                        ...
                        <h2 className="text-4xl font-black text-[#1a1025] tracking-tight mb-3">Pilih Paket Hosting</h2>
                        <p className="text-[#666] text-[15px]">Transparan. Tanpa biaya tersembunyi.</p>
                    </div>

                    <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-px bg-[#e5e5e5] border border-[#e5e5e5] mb-16" data-reveal>
                        {activePlans.map(([slug, plan]) => {
                            const p = planPricing[slug];
                            const pop = slug === 'pro';
                            const pc = pop ? planColors.violet : planColors.default;
                            return (
                                <div key={slug} className={`${pc.card} p-8 flex flex-col relative`}>
                                    {pop && <div className={`${pc.badge} text-white text-[10px] font-bold tracking-widest uppercase text-center py-1.5 -mx-8 -mt-8 mb-6`}>Paling Populer</div>}
                                    <h3 className={`text-lg font-bold mb-4 ${pop ? 'text-white' : 'text-[#1a1025]'}`}>{plan.label}</h3>
                                    <div className="mb-6">
                                        {p.promo !== null && <span className={`text-xs price-strike block mb-1 ${pop ? 'text-[#a78bfa]' : 'text-[#999]'}`}>Rp {rupiah(p.normal)}</span>}
                                        {p.promo === null && <div className="h-4 mb-1"></div>}
                                        <div className="flex items-baseline gap-1">
                                            <span className={`text-3xl font-black tracking-tight ${pop ? 'text-white' : 'text-[#1a1025]'}`}>Rp {rupiah(p.active)}</span>
                                            <span className={`text-sm ${pop ? 'text-[#a78bfa]' : 'text-[#999]'}`}>/bln</span>
                                        </div>
                                    </div>
                                    <ul className="space-y-2.5 flex-1">
                                        {plan.features.map(f => (
                                            <li key={f} className={`flex items-start gap-2.5 text-[13px] ${pop ? 'text-[#d4c4f0]' : 'text-[#666]'}`}>
                                                <i className={`fa-solid fa-check text-[9px] mt-[5px] ${pop ? 'text-[#a78bfa]' : 'text-[#7c3aed]'}`}></i>
                                                <span>{f}</span>
                                            </li>
                                        ))}
                                    </ul>
                                    <a href="/register" className={`mt-8 flex items-center justify-center w-full py-2.5 text-sm font-semibold transition-colors ${pop ? pc.btn : 'bg-[#1a1025] text-white hover:bg-[#2d1f42]'}`}>
                                        Pilih Paket
                                    </a>
                                </div>
                            );
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-[#e5e5e5] border border-[#e5e5e5]" data-reveal>
                        {[
                            ...
                            <div key={f.l} className="bg-[#fafafa] flex items-center justify-center gap-3 py-5">
                                <i className={`fa-solid ${f.i} text-[#7c3aed] text-sm`}></i>
                                <span className="text-sm font-semibold text-[#1a1025]">{f.l}</span>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="max-w-6xl mx-auto px-6 py-24 lg:py-32">
                    <div className="flex items-end justify-between mb-12" data-reveal>
                        <div>
                            ...
                            <h2 className="text-4xl font-black text-[#1a1025] tracking-tight">Artikel Terbaru</h2>
                        </div>
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-px bg-[#e5e5e5] border border-[#e5e5e5]" data-reveal>
                        {articles.length > 0 ? articles.map(a => (
                            <a key={a.id} href={a.url} className="bg-white group">
                                {a.cover_image ? (
                                    <div className="h-44 overflow-hidden bg-[#f5f5f5]"><img src={a.cover_image} alt={a.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" /></div>
                                ) : (
                                    <div className="h-44 bg-[#f5f5f5] flex items-center justify-center text-[#ccc]"><i className="fa-solid fa-newspaper text-4xl"></i></div>
                                )}
                                <div className="p-6">
                                    {a.category && <span className="text-[10px] font-bold text-[#7c3aed] uppercase tracking-widest">{a.category.name}</span>}
                                    <h3 className="text-[15px] font-bold text-[#1a1025] mt-2 mb-2 group-hover:text-[#7c3aed] transition-colors line-clamp-2">{a.title}</h3>
                                    <p className="text-sm text-[#999] line-clamp-2 mb-4">{a.excerpt || ''}</p>
                                    <div className="flex items-center gap-3 text-xs text-[#bbb] pt-4 border-t border-[#f0f0f0]">
                                        <span>{a.published_at}</span><span>&middot;</span><span>{a.reading_time} mnt</span>
                                    </div>
                                </div>
                            </a>
                        )) : (
                            <div className="bg-white py-16 text-center col-span-full text-sm text-[#999]">Belum ada artikel.</div>
                        )}
                    </div>
            ...
            {/* ═══ FAQ ═══ */}
            <section id="faq" className="bg-white border-y border-[#e5e5e5]">
                <div className="max-w-3xl mx-auto px-6 py-24 lg:py-32">
                    <div className="mb-12" data-reveal>
                        ...
                        <h2 className="text-4xl font-black text-[#1a1025] tracking-tight">Pertanyaan Umum</h2>
                    </div>
                    <div data-reveal>
            ...
            <div className="fixed bottom-6 right-6 z-50">
                {chatOpen && (
                    <div className="bg-white border border-[#e5e5e5] shadow-2xl w-80 h-96 mb-4 flex flex-col overflow-hidden">
                        ...
                        <div className="flex-1 p-4 overflow-y-auto space-y-3 bg-[#fafafa]">
                            {chatMsgs.length === 0 && (
                                <div className="flex items-start gap-2">
                                    <div className="w-6 h-6 bg-[#f5f0ff] flex items-center justify-center shrink-0"><i className="fa-solid fa-robot text-[10px] text-[#7c3aed]"></i></div>
                                    <div className="bg-white border border-[#e5e5e5] px-3 py-2 text-sm text-[#444] max-w-[85%]">Halo! Saya asisten AI Ryaze. Ada yang bisa saya bantu?</div>
                                </div>
                            )}
                            {chatMsgs.map(m => (
                                <div key={m.id} className={`flex items-start gap-2 ${m.user ? 'flex-row-reverse' : ''}`}>
                                    <div className={`w-6 h-6 flex items-center justify-center shrink-0 ${m.user ? 'bg-[#e5e5e5]' : 'bg-[#f5f0ff]'}`}>
                                        <i className={`fa-solid ${m.user ? 'fa-user text-[10px] text-[#999]' : 'fa-robot text-[10px] text-[#7c3aed]'}`}></i>
                                    </div>
                                    <div className={`px-3 py-2 text-sm max-w-[85%] ${m.user ? 'bg-[#1a1025] text-white' : 'bg-white border border-[#e5e5e5] text-[#444]'}`}>{m.text}</div>
                                </div>
                            ))}
                            {chatLoading && (
                                <div className="flex items-start gap-2">
                                    <div className="w-6 h-6 bg-[#f5f0ff] flex items-center justify-center shrink-0"><i className="fa-solid fa-robot text-[10px] text-[#7c3aed]"></i></div>
                                    <div className="bg-white border border-[#e5e5e5] px-4 py-3 flex gap-1">
                                        <div className="w-1.5 h-1.5 bg-[#ccc] rounded-full animate-bounce"></div>
                                        <div className="w-1.5 h-1.5 bg-[#ccc] rounded-full animate-bounce" style={{ animationDelay: '0.1s' }}></div>
                                        <div className="w-1.5 h-1.5 bg-[#ccc] rounded-full animate-bounce" style={{ animationDelay: '0.2s' }}></div>
                                    </div>
                                </div>
                            )}
                        </div>
                        <form onSubmit={sendChat} className="p-3 bg-white border-t border-[#e5e5e5] flex gap-2">
                            <input type="text" value={chatInput} onChange={e => setChatInput(e.target.value)} placeholder="Ketik pesan..." required className="flex-1 bg-[#fafafa] border border-[#e5e5e5] text-sm px-4 py-2 focus:outline-none focus:border-[#7c3aed] transition-colors" />
                ...
                <button onClick={() => setChatOpen(!chatOpen)} className="w-14 h-14 bg-[#7c3aed] hover:bg-[#6d28d9] text-white rounded-full flex items-center justify-center shadow-lg shadow-[#7c3aed]/30 transition-all hover:scale-105 ml-auto relative">
                    ...
                    {!chatOpen && <span className="absolute -top-0.5 -right-0.5 w-3 h-3 bg-rose-500 border-2 border-white rounded-full"></span>}
                </button>
            </div>
        </PublicLayout>
    );
