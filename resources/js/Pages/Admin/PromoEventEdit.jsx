...
export default function PromoEventEdit() {
    const { promo_event } = usePage().props;
    const { data, setData, post, processing, errors } = useForm({
        title: promo_event?.title || '',
        description: promo_event?.description || '',
        discount_percent: promo_event?.discount_percent || '',
        start_date: promo_event?.start_date?.split('T')[0] || '',
        end_date: promo_event?.end_date?.split('T')[0] || '',
        is_active: promo_event?.is_active ?? true,
        _method: 'put',
    });

    const handleSubmit = (e) => {
        e.preventDefault();
        post(`/superadmin/promo-events/${promo_event?.hashid || promo_event?.id}`);
    };
