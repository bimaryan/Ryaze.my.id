...
export default function PortfolioEdit({ portfolio }) {
    const { data, setData, post, processing, errors } = useForm({
        ...
        link_preview: portfolio.link_preview || '',
        link_github: portfolio.link_github || '',
        link_journal: portfolio.link_journal || '',
        image: null,
        certificate: null,
        link_copyright: portfolio.link_copyright || '',
        is_active: portfolio.is_active || false,
        _method: 'PUT',
    });

    const [imagePreview, setImagePreview] = useState(
        portfolio.image_path ? `/storage/${portfolio.image_path}` : null
    );
    ...
    function handleSubmit(e) {
        e.preventDefault();
        post(route('superadmin.portfolios.update', { portfolio: portfolio.hashid }), { forceFormData: true });
    }
        ...
        <DashboardLayout title="Edit Portofolio">
            <div className="mb-1">
                <div className="flex items-center gap-3 mb-1">
                    ...
                    <div>
                        <h1 className="text-lg font-bold text-[#333] dark:text-white">Edit Portofolio</h1>
                        <p className="text-[13px] text-[#999] dark:text-white/40">Ubah data portofolio/mahakarya yang sudah ada.</p>
                    </div>
                ...
                <form onSubmit={handleSubmit} encType="multipart/form-data">
                    <div className="space-y-6">
                        ...
                        <div>
                            ...
                            {imagePreview && (
                                <div className="mb-3">
                                    ...
                                    <img src={imagePreview} alt={portfolio.title} className="w-48 rounded-lg border border-[#e5e5e5] dark:border-[#1a1a2e] shadow-sm" />
                                </div>
                            )}
                            ...
                            <p className="text-xs text-[#999] dark:text-white/40 mt-2">Format: JPG, PNG, WEBP (Max 2MB). Kosongkan jika tidak ingin mengubah gambar.</p>
                        ...
                        <div>
                            ...
                            {portfolio.certificate_path && (
                                <div className="mb-3 flex items-center gap-3 p-3 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/40 rounded-lg">
                                    <i className="fa-solid fa-file-certificate text-amber-500 dark:text-amber-400 text-xl"></i>
                                    <div>
                                        <p className="text-xs text-[#999] dark:text-white/40 mb-1">Sertifikat saat ini:</p>
                                        <a href={`/storage/${portfolio.certificate_path}`} target="_blank" rel="noopener noreferrer" className="text-sm text-amber-700 dark:text-amber-300 font-medium hover:underline">
                                            <i className="fa-solid fa-external-link mr-1"></i> Lihat Sertifikat
                                        </a>
                                    </div>
                                </div>
                            )}
                            ...
                            <p className="text-xs text-[#999] dark:text-white/40 mt-2">Format: PDF, JPG, PNG (Max 5MB). Kosongkan jika tidak ingin mengubah sertifikat.</p>
                        ...
                        <div className="pt-4 border-t border-[#e5e5e5] dark:border-[#1a1a2e] flex justify-end">
                            <button type="submit" disabled={processing} className="bg-[#7c3aed] hover:bg-[#6d28d9] text-white font-bold py-2.5 px-6 rounded-lg shadow-md transition disabled:opacity-50">
                                <i className="fa-solid fa-save mr-2"></i>{processing ? 'Menyimpan...' : 'Perbarui Portofolio'}
                            </button>
                        </div>
                    </div>
                </form>
