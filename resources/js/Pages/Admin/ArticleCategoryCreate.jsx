...
export default function ArticleCategoryCreate() {
    ...
    function handleSubmit(e) {
        e.preventDefault();
        post(route('superadmin.article_categories.store'));
    }
            ...
            <div className="mb-1">
                <div className="flex items-center gap-3 mb-1">
                    <div className="w-10 h-10 bg-[#f5f0ff] dark:bg-[#7c3aed]/20 flex items-center justify-center">
                        <i className="fa-solid fa-folder-plus text-[#7c3aed] dark:text-[#a78bfa]"></i>
                    </div>
                    <div>
                        <h1 className="text-lg font-bold text-[#333] dark:text-white">Tambah Kategori</h1>
                        <p className="text-[13px] text-[#999] dark:text-white/40">Buat kategori baru untuk mengelompokkan artikel.</p>
                    </div>
                ...
                <div className="bg-white dark:bg-[#0d0d18] border border-[#e5e5e5] dark:border-[#1a1a2e] rounded-xl shadow-sm p-6">
                    <form onSubmit={handleSubmit}>
                        <div className="mb-5">
                            ...
                            <input type="text" value={data.name} onChange={(e) => setData('name', e.target.value)} required className={inputCls} placeholder="Contoh: Tutorial" />
                            {errors.name && <p className="text-red-500 dark:text-red-400 text-xs mt-1">{errors.name}</p>}
                        </div>
                        <div className="mb-5">
                            <label className="block mb-1.5 text-sm font-medium text-[#666] dark:text-white/60">Deskripsi</label>
                            <textarea value={data.description} onChange={(e) => setData('description', e.target.value)} rows="3" className={inputCls} placeholder="Deskripsi singkat kategori (opsional)" />
                        ...
                        <button type="submit" disabled={processing} className="bg-[#7c3aed] hover:bg-[#6d28d9] text-white font-semibold text-sm py-3 px-6 rounded-lg transition shadow-sm disabled:opacity-50">
                            <i className="fa-solid fa-save mr-2"></i>{processing ? 'Menyimpan...' : 'Simpan Kategori'}
                        </button>
                    </form>
                </div>
            </div>
