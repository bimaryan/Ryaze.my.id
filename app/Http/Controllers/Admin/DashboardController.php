...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        return inertia('Dashboard/Superadmin', [
            ...
            'recentUsers' => $recentUsers->map(fn ($u) => ['id' => $u->id, 'hashid' => $u->hashid, 'name' => $u->name, 'email' => $u->email, 'role' => $u->role, 'created_at' => $u->created_at->diffForHumans()]),
            'recentJokiOrders' => $recentJokiOrders->map(fn ($o) => ['id' => $o->id, 'order_number' => $o->order_number, 'project_name' => $o->project_name, 'status' => $o->status, 'price' => $o->price ?? 0, 'client_name' => $o->client?->name ?? '-', 'service_name' => $o->service?->name ?? '-', 'created_at' => $o->created_at->format('d M Y')]),
            'recentHostingProjects' => $recentHostingProjects->map(fn ($p) => ['id' => $p->id, 'hashid' => $p->hashid, 'project_name' => $p->project_name, 'framework' => $p->framework, 'ryaze_domain' => $p->ryaze_domain, 'status' => $p->status, 'client_name' => $p->client?->name ?? '-', 'created_at' => $p->created_at->format('d M Y')]),
